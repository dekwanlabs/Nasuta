...
- **Push data work into SQL**: filtering, sorting, aggregation, dedup, projection, pagination, and joins belong in the query whenever SQL can express them at acceptable complexity — never load raw rows and then re-process them in Go. Use Go only for what SQL cannot express (custom scoring, cross-row derivations), and then in exactly one pass, not scattered post-processing.
- **One source of truth per concern — ordering**: let the database `ORDER BY`; don't re-sort the same rows in Go. Before deleting a Go sort, check the two aren't different concerns — a SQL `ORDER BY` feeding a keyset cursor or a `LIMIT` truncation, or a Go `sort` normalizing order for a stable hash, is not redundancy.
- **One source of truth per concern — derived values**: a column/aggregate computed in SQL (`CASE`, `JSON_EXTRACT`, `COUNT`, `MAX`) must not be recomputed or overwritten in Go after scanning. Cross-row derivations (inheritance, lineage) belong in a recursive CTE or one Go pass — never both.
- **One operation, one method, one type**: don't stack `X()` (error-swallowing map), `XResult()` (map + error), `FindX()` (domain type), `SearchX()` (facade type) for the same logic. Return errors; delete the swallowing wrapper.
- **Parallel types need a real boundary**: near-1:1 structs across packages plus manual `toXxx` converters are duplication unless the boundary is real — a stable public facade, hiding secrets, or a write-model/read-model projection. A zero-reference type alias is dead; delete it.
- **Interfaces and adapters are not redundancy**: an interface re-declaring a concrete type's methods (for mock/decoupling) or a provider adapter forwarding to a client (implementing a backend interface) is intentional structure — don't collapse it.
- **Caches need an invalidation path**: an in-memory copy of DB data must have a reset hook that runs on rebuild; don't hold two caches of the same table — reuse the existing invalidatable one.
- **Delete dead code defensively**: grep every reference (tests included, plus bare names within the same package) before deleting — don't trust an audit or intuition. Conversely, a "provably redundant" recomputation may be a defensive invariant (e.g. guarding against concurrent writes); weigh keeping it.
