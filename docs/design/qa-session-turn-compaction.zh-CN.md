...
4. 单轮 `detail_json` 最多约 2400 token，超限时持续收紧用户、调用、结果和回答预算，必须校验最终一次收缩结果；
5. 工具事件的固定元数据仍超过预算时，保留首尾事件并通过 `omittedToolCalls`、`omittedToolResults` 记录省略数量；原始消息继续保存在 `qa_messages`；
6. 无法生成合法且受限的 JSON 时，本次压缩失败，不写入半成品，也不在错误日志打印整轮内容。
