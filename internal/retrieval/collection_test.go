...
func TestCollectRunbooksUsesMatchedChunksAndDeduplicates(t *testing.T) {
	...
	record := types.RunbookRecord{Title: "flow", Scope: "event", Text: "FULL DOCUMENT MUST NOT BE INJECTED"}
	hits := []types.RunbookSearchHit{
...
func TestCollectRunbooksBoundsMergedChunksByRunes(t *testing.T) {
	retrieve := New(nil, config.Config{})
	hit := types.RunbookSearchHit{
		Record:        types.RunbookRecord{Title: "flow"},
		ChunkText:     strings.Repeat("证", 5000),
		Score:         0.9,
		SemanticScore: 0.9,
	}
	var docs []codeDoc
	retrieve.collectRunbooks(context.Background(), []types.RunbookSearchHit{hit}, func(doc codeDoc) { docs = append(docs, doc) })
	if len(docs) != 1 {
		t.Fatalf("docs = %d, want 1", len(docs))
	}
