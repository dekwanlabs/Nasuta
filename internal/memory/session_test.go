...
func TestListMessagesBeforeUsesExclusiveCursor(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT m\.seq, m\.role, m\.content.*m\.seq < \?.*ORDER BY m\.seq DESC LIMIT \?`).
		...
		WillReturnRows(sqlmock.NewRows([]string{"seq", "role", "content", "tool_calls_json", "tool_call_id", "tool_name", "feedback", "created_at", "run_id"}).
			AddRow(3, "assistant", "three", "", "", "", "like", createdAt, "run-2").
			AddRow(2, "user", "two", "", "", "", "", createdAt, "").
			AddRow(1, "assistant", "one", "", "", "", "dislike", createdAt, "run-1"))
	...
	if page.Messages[0].Seq != 2 || page.Messages[1].Seq != 3 || page.Messages[1].Feedback != "like" {
...
func TestListTurnsBeforeKeepsCompleteTurns(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT m\.seq, m\.role, m\.content.*m\.seq >= \?.*m\.seq < \?.*ORDER BY m\.seq ASC`).
		...
		WillReturnRows(sqlmock.NewRows([]string{"seq", "role", "content", "tool_calls_json", "tool_call_id", "tool_name", "feedback", "created_at", "run_id"}).
			...
			AddRow(7, "assistant", "answer three", "", "", "", "dislike", createdAt, "run-3"))
	...
	if page.Messages[len(page.Messages)-1].Seq != 7 || page.Messages[len(page.Messages)-1].Feedback != "dislike" {
		t.Fatalf("final answer metadata = %#v", page.Messages[len(page.Messages)-1])
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
	}
...
func TestSetMessageFeedbackBySequence(t *testing.T) {
	store, mock, closeDB := newMockSessionStore(t)
	defer closeDB()
	mock.ExpectBegin()
	...
	updated, err := store.SetMessageFeedback("session-1", 42, 7, "", "like")
	if err != nil || !updated {
...
func TestSetMessageFeedbackByRun(t *testing.T) {
	store, mock, closeDB := newMockSessionStore(t)
	defer closeDB()
	mock.ExpectBegin()
	mock.ExpectQuery(`SELECT m\.id, m\.feedback.*JOIN qa_turns.*t\.run_id=\?.*FOR UPDATE`).
		WithArgs("session-1", int64(42), "run-3").
		WillReturnRows(sqlmock.NewRows([]string{"id", "feedback"}).AddRow(17, "like"))
	mock.ExpectExec(`UPDATE qa_messages SET feedback=\? WHERE id=\?`).
		WithArgs("", int64(17)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()

	updated, err := store.SetMessageFeedback("session-1", 42, 0, "run-3", "")
	if err != nil || !updated {
		t.Fatalf("updated=%v err=%v", updated, err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
