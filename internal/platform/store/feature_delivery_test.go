...
func TestAppendRunEventsUsesOneSequenceAllocationAndInsert(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT COALESCE\(MAX\(seq\),0\)\+1 FROM runtime_events\s+WHERE stream_kind='feature_implementation' AND stream_id=\?`).
		WithArgs("run-1").
		WillReturnRows(sqlmock.NewRows([]string{"seq"}).AddRow(7))
	mock.ExpectExec(`INSERT INTO runtime_events.*VALUES\('feature_implementation',\?,\?,\?,\?,\?,\?,\?\),\('feature_implementation',\?,\?,\?,\?,\?,\?,\?\)`).
		WithArgs(
			"run-1", int64(7), delivery.EventProviderMessage, "", "first", nil, createdAt,
			"run-1", int64(8), delivery.EventCommandStarted, "", "second", []byte(`{"command":"go test"}`), createdAt,
		).
		WillReturnResult(sqlmock.NewResult(0, 2))
	mock.ExpectCommit()

	store := NewFeatureDeliveryStore(db)
	events, err := store.AppendRunEvents(context.Background(), []delivery.RunEvent{
		{RunID: "run-1", Kind: delivery.EventProviderMessage, Summary: "first", CreatedAt: createdAt},
		{RunID: "run-1", Kind: delivery.EventCommandStarted, Summary: "second", Detail: json.RawMessage(`{"command":"go test"}`), CreatedAt: createdAt},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCurrentParentArtifactIDUsesApprovedLineage(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT id FROM feature_artifacts.*kind=\?.*ORDER BY version DESC LIMIT 1`).
		WithArgs("feat-1", delivery.KindRequirement).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow("req-2"))
	mock.ExpectQuery(`(?s)SELECT a.id FROM feature_artifacts a.*a.parent_artifact_id=\?.*a.review_decision='approved'.*ORDER BY a.version DESC LIMIT 1`).
		WithArgs("feat-1", delivery.KindRequirementAnalysis, "req-2").
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow("analysis-3"))
	mock.ExpectQuery(`(?s)SELECT a.id FROM feature_artifacts a.*a.parent_artifact_id=\?.*a.review_decision='approved'.*ORDER BY a.version DESC LIMIT 1`).
		WithArgs("feat-1", delivery.KindTechnicalProposal, "analysis-3").
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow("proposal-4"))

	parentID, err := currentParentArtifactID(context.Background(), db, "feat-1", delivery.KindSystemDesign)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCurrentParentArtifactIDRejectsIncompleteLineage(t *testing.T) {
	...
	_, err = currentParentArtifactID(context.Background(), db, "feat-1", delivery.KindTechnicalProposal)
...
func TestReviewArtifactLocksFeatureBeforeArtifact(t *testing.T) {
	...
	subject, err := delivery.BuildArtifactReviewSubject(delivery.Artifact{
		ID: "analysis-1", Kind: delivery.KindRequirementAnalysis, Version: 1,
		ParentArtifactID: "req-1", Evidence: []delivery.EvidenceRef{},
	...
	mock.ExpectQuery(`(?s)SELECT kind,parent_artifact_id,version,content_hash,evidence_json.*WHERE id=\? AND request_id=\? LIMIT 1 FOR UPDATE`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"review_subject_hash", "review_round_id", "review_gate_result_id",
			"review_decision", "review_comment", "review_reviewer", "review_created_at",
		}).AddRow(
			delivery.KindRequirementAnalysis, "req-1", 1, "artifact-hash", []byte(`[]`),
			nil, nil, nil, nil, nil, nil, nil,
		))
	mock.ExpectQuery(`(?s)SELECT id FROM feature_artifacts.*kind=\?.*ORDER BY version DESC LIMIT 1`).
		WithArgs("feat-1", delivery.KindRequirement).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow("req-1"))
	mock.ExpectExec(`(?s)UPDATE feature_artifacts.*SET review_subject_hash=\?,review_round_id=\?,review_gate_result_id=\?,review_decision=\?.*WHERE id=\? AND review_subject_hash IS NULL`).
		WithArgs(subject.ContentHash, "round-1", "gate-1", delivery.DecisionApproved, "approved", int64(42), reviewedAt, "analysis-1").
		WillReturnResult(sqlmock.NewResult(0, 1))
	...
	store := NewFeatureDeliveryStore(db)
	err = store.ReviewArtifact(context.Background(), delivery.ArtifactReview{
		ArtifactID:  "analysis-1",
		SubjectHash: subject.ContentHash, ReviewRoundID: "round-1", GateResultID: "gate-1",
		Decision:  delivery.DecisionApproved,
		Comment:   "approved",
		Reviewer:  42,
...
func TestReviewArtifactRejectsDifferentExistingFact(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
	...
	err = NewFeatureDeliveryStore(db).ReviewArtifact(
		...
		delivery.ArtifactReview{
			ArtifactID: "analysis-1", SubjectHash: subject.ContentHash,
			ReviewRoundID: "round-1", GateResultID: "gate-new",
			Decision: delivery.DecisionApproved, Comment: "approved",
			Reviewer: 42, CreatedAt: reviewedAt,
		},
	)
	if !errors.Is(err, delivery.ErrConflict) {
...
func TestListArtifactsUsesSummaryColumnsAndCursor(t *testing.T) {
	...
	mock.ExpectQuery(query).
		WithArgs("feat-1", delivery.KindRequirementAnalysis, 3, 20).
		WillReturnRows(sqlmock.NewRows([]string{
			"id", "request_id", "kind", "version", "parent_artifact_id", "origin",
			"content_hash", "created_by", "created_at", "review_subject_hash", "review_round_id",
			"review_gate_result_id", "review_decision", "review_comment", "review_reviewer", "review_created_at",
		}).AddRow(
			"analysis-2", "feat-1", delivery.KindRequirementAnalysis, 2, "req-1", delivery.OriginAgent,
			"hash", int64(7), createdAt, nil, nil, nil, nil, nil, nil, nil,
		))

	items, err := NewFeatureDeliveryStore(db).ListArtifacts(
		context.Background(), "feat-1", delivery.KindRequirementAnalysis,
		delivery.ArtifactCursor{Kind: delivery.KindRequirementAnalysis, Version: 3}, 20,
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetCurrentLineageUsesOneBoundedQuery(t *testing.T) {
	...
	rows := sqlmock.NewRows(columns).
		AddRow("req-2", "feat-1", delivery.KindRequirement, 2, "", delivery.OriginUser,
			[]byte(`{"description":"request"}`), "request", []byte(`[]`), "req-hash", int64(7), createdAt,
			nil, nil, nil, nil, nil, nil, nil).
		AddRow("analysis-3", "feat-1", delivery.KindRequirementAnalysis, 3, "req-2", delivery.OriginAgent,
			[]byte(`{"background":"analysis"}`), "analysis", []byte(`[]`), "analysis-hash", int64(7), createdAt,
			"review-hash", "round-1", "gate-1", delivery.DecisionApproved, "approved", int64(9), createdAt)
...
func TestCompleteGenerationCommitsArtifactAndRunTogether(t *testing.T) {
	...
	artifact := delivery.Artifact{
		ID: "analysis-1", RequestID: "feat-1", Kind: delivery.KindRequirementAnalysis,
		ParentArtifactID: "req-1", Origin: delivery.OriginAgent,
		DocumentJSON: []byte(`{}`), RenderedMarkdown: "analysis", Evidence: []delivery.EvidenceRef{},
	...
	mock.ExpectQuery(`(?s)SELECT request_id,artifact_kind,parent_artifact_id,status.*WHERE id=\? LIMIT 1 FOR UPDATE`).
		...
		AddRow("feat-1", delivery.KindRequirementAnalysis, "req-1", "running"))
	mock.ExpectQuery(`(?s)SELECT id FROM feature_artifacts.*kind=\?.*ORDER BY version DESC LIMIT 1`).
		WithArgs("feat-1", delivery.KindRequirement).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow("req-1"))
	mock.ExpectQuery(`SELECT COALESCE\(MAX\(version\),0\)\+1 FROM feature_artifacts WHERE request_id=\? AND kind=\?`).
		WithArgs("feat-1", delivery.KindRequirementAnalysis).
		WillReturnRows(sqlmock.NewRows([]string{"version"}).AddRow(4))
	mock.ExpectExec(`INSERT INTO feature_artifacts`).
		WithArgs(
			"analysis-1", "feat-1", delivery.KindRequirementAnalysis, 4, "req-1",
			delivery.OriginAgent, []byte(`{}`), "analysis", []byte(`[]`), "hash", int64(7), createdAt,
...
func TestListGenerationRunsUsesStableCursorAndLimit(t *testing.T) {
	...
	mock.ExpectQuery(query).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"gen-2", "feat-1", delivery.KindTechnicalProposal, "analysis-1", "",
	...
	runs, err := NewFeatureDeliveryStore(db).ListGenerationRuns(
		context.Background(), "feat-1", delivery.GenerationCursor{StartedAt: startedAt, ID: "gen-3"}, 20,
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetSuccessfulGenerationForWorkflowNodeUsesLatestAttempt(t *testing.T) {
	...
	mock.ExpectQuery(query).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"gen-2", "feat-1", delivery.KindRequirementAnalysis, "req-1", "workflow-1",
...
func TestTransitionImplementationRejectsInvalidLifecycle(t *testing.T) {
	...
	err = store.TransitionImplementation(
		context.Background(), "run-1", "worker-1", delivery.RunPreparing,
		delivery.RunSucceeded, delivery.RunUpdate{},
	)
	if !errors.Is(err, delivery.ErrInvalid) {
		t.Fatalf("invalid transition error = %v", err)
	}
	err = store.TransitionImplementation(
		context.Background(), "run-1", "worker-1", delivery.RunRunning,
		delivery.RunFailed, delivery.RunUpdate{},
	)
	if !errors.Is(err, delivery.ErrInvalid) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			run := delivery.ImplementationRun{
				...
				Provider: "codex", Status: delivery.RunQueued, RequestedBy: 42, CreatedAt: createdAt,
			}
			mock.ExpectBegin()
			...
			mock.ExpectQuery(`(?s)FROM feature_implementation_runs r.*WHERE r.requested_by=\? AND r.client_request_id=\? LIMIT 1`).
				...
				WillReturnRows(implementationRunRows(delivery.ImplementationRun{
			...
			if test.wantConflict {
				if !errors.Is(err, delivery.ErrConflict) || saved != nil || created {
...
func TestMutableFeatureWritesRejectArchivedRequests(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			run: func(store *FeatureDeliveryStore) error {
				_, err := store.CreateArtifact(context.Background(), delivery.Artifact{
					ID: "artifact-1", RequestID: "feat-1", Kind: delivery.KindRequirement,
				})
				return err
			},
		},
		{
			name: "generation",
			run: func(store *FeatureDeliveryStore) error {
				return store.CreateGenerationRun(context.Background(), delivery.GenerationRun{
		...
		},
		{
			name: "implementation",
			run: func(store *FeatureDeliveryStore) error {
				_, _, err := store.CreateImplementation(context.Background(), delivery.ImplementationRun{
					ID: "run-1", RequestID: "feat-1", Status: delivery.RunQueued,
				})
				return err
			},
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := test.run(NewFeatureDeliveryStore(db)); !errors.Is(err, delivery.ErrConflict) {
...
func TestListImplementationsUsesSummaryColumnsAndStableCursor(t *testing.T) {
	...
	mock.ExpectQuery(query).
		...
		WillReturnRows(implementationSummaryRows(delivery.ImplementationRun{
			ID: "run-2", RequestID: "feat-1", ClientRequestID: "client-2", RequestHash: "hash",
			DesignArtifactID: "design-1", PlanArtifactID: "plan-1", Repo: "team/service",
			BaseRef: "main", BaseCommit: "abc", WorkspaceUserID: 7, WorkspaceUsername: "developer",
			Provider: "codex", Status: delivery.RunSucceeded, RequestedBy: 42,
			CreatedAt: createdAt.Add(-time.Minute),
			Review: &delivery.ChangeReview{
				RunID: "run-2", SubjectHash: "review-hash", ReviewRoundID: "round-1", GateResultID: "gate-1",
				Decision: delivery.DecisionRejected, Comment: "revise", Reviewer: 9, CreatedAt: createdAt,
			},
		}))

	runs, err := NewFeatureDeliveryStore(db).ListImplementations(
		context.Background(), "feat-1", delivery.RunCursor{CreatedAt: createdAt, ID: "run-3"}, 20,
	)
	if err != nil {
		t.Fatal(err)
	}
	if len(runs) != 1 || runs[0].ID != "run-2" || runs[0].ChangeSet != nil ||
		runs[0].Review == nil || runs[0].Review.Decision != delivery.DecisionRejected {
...
func TestGetImplementationLoadsEmbeddedChangeSet(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	createdAt := time.Date(2026, 7, 29, 12, 50, 0, 0, time.UTC)
	changeCreatedAt := createdAt.Add(time.Minute)
	run := delivery.ImplementationRun{
		ID: "run-1", RequestID: "feat-1", ClientRequestID: "client-1", RequestHash: "hash",
		DesignArtifactID: "design-1", PlanArtifactID: "plan-1", Repo: "team/service",
		BaseRef: "main", BaseCommit: "abc", WorkspaceUserID: 7, WorkspaceUsername: "developer",
		Provider: "codex", Status: delivery.RunSucceeded, RequestedBy: 42, CreatedAt: createdAt,
		ChangeSet: &delivery.ChangeSet{
			RunID: "run-1", WorktreeHead: "def", PatchRelPath: "run-1/changes.patch",
			PatchSHA256: "patch-hash", PatchBytes: 42, FilesChanged: 1, Additions: 2, Deletions: 1,
			Files:             []delivery.ChangedFile{{Path: "main.go", Status: "M"}},
			PlanDeviations:    []delivery.PlanDeviation{},
			ValidationResults: []delivery.ValidationResult{{Sequence: 1, Status: "passed"}},
			ProviderSummary:   "implemented", CreatedAt: changeCreatedAt,
		},
	}
	mock.ExpectQuery(`(?s)FROM feature_implementation_runs r.*WHERE r.id=\? LIMIT 1`).
		WithArgs("run-1").
		WillReturnRows(implementationRunRows(run))

	got, err := NewFeatureDeliveryStore(db).GetImplementation(context.Background(), "run-1")
	if err != nil {
		t.Fatal(err)
	}
	if got.ChangeSet == nil || got.ChangeSet.RunID != "run-1" ||
		got.ChangeSet.PatchSHA256 != "patch-hash" ||
		len(got.ChangeSet.ValidationResults) != 1 {
		t.Fatalf("embedded change set = %+v", got.ChangeSet)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
	}
...
func TestReviewChangeSetStoresReviewOnImplementation(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
	...
	reviewedAt := time.Date(2026, 7, 29, 12, 55, 0, 0, time.UTC)
	run := delivery.ImplementationRun{
		ID: "run-1", RequestID: "feat-1", Repo: "team/service",
		BaseCommit: "abc", Status: delivery.RunSucceeded,
		ChangeSet: &delivery.ChangeSet{
			RunID: "run-1", WorktreeHead: "def", PatchSHA256: "patch-hash",
			Files:           []delivery.ChangedFile{{Path: "main.go", Status: "M"}},
			PlanDeviations:  []delivery.PlanDeviation{},
			ProviderSummary: "implemented",
		},
	}
	subject, err := delivery.BuildChangeSetReviewSubject(run)
	if err != nil {
		t.Fatal(err)
	}

	mock.ExpectBegin()
	mock.ExpectQuery(`SELECT request_id,status FROM feature_implementation_runs WHERE id=\? LIMIT 1 FOR UPDATE`).
		WithArgs("run-1").
		WillReturnRows(sqlmock.NewRows([]string{"request_id", "status"}).
			AddRow("feat-1", delivery.RunSucceeded))
	mock.ExpectQuery(`(?s)SELECT repo,base_commit,worktree_head,patch_sha256,.*FROM feature_implementation_runs.*WHERE id=\? AND change_set_created_at IS NOT NULL LIMIT 1 FOR UPDATE`).
		WithArgs("run-1").
		WillReturnRows(changeSetReviewRows(run))
	mock.ExpectExec(`(?s)UPDATE feature_implementation_runs.*SET review_subject_hash=\?,review_round_id=\?,review_gate_result_id=\?,review_decision=\?.*WHERE id=\? AND review_subject_hash IS NULL`).
		WithArgs(
			subject.ContentHash, "round-1", "gate-1", delivery.DecisionApproved,
			"approved", int64(42), reviewedAt, "run-1",
		).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec(`UPDATE feature_requests SET updated_at=CURRENT_TIMESTAMP WHERE id=\?`).
		WithArgs("feat-1").
	...
	err = NewFeatureDeliveryStore(db).ReviewChangeSet(context.Background(), delivery.ChangeReview{
		RunID: "run-1", SubjectHash: subject.ContentHash,
		ReviewRoundID: "round-1", GateResultID: "gate-1",
		Decision: delivery.DecisionApproved, Comment: "approved",
		Reviewer: 42, CreatedAt: reviewedAt,
	})
...
func TestReviewChangeSetRejectsExistingReview(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
	...
	err = NewFeatureDeliveryStore(db).ReviewChangeSet(context.Background(), *run.Review)
	if !errors.Is(err, delivery.ErrConflict) {
		t.Fatalf("existing change review error = %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
	}
...
func changeSetReviewRows(run delivery.ImplementationRun) *sqlmock.Rows {
	change := run.ChangeSet
	filesJSON, _ := json.Marshal(change.Files)
	deviationsJSON, _ := json.Marshal(change.PlanDeviations)
	validationsJSON, _ := json.Marshal(change.ValidationResults)
	var subjectHash, reviewRoundID, gateResultID, decision, comment any
	var reviewer, reviewedAt any
	if run.Review != nil {
		...
		reviewedAt = run.Review.CreatedAt
	}
	return sqlmock.NewRows([]string{
		"repo", "base_commit", "worktree_head", "patch_sha256",
		"files_json", "plan_deviations_json", "validation_results_json", "provider_summary",
		"review_subject_hash", "review_round_id", "review_gate_result_id",
		"review_decision", "review_comment", "review_reviewer", "review_created_at",
	}).AddRow(
		run.Repo, run.BaseCommit, change.WorktreeHead, change.PatchSHA256,
		filesJSON, deviationsJSON, validationsJSON, change.ProviderSummary,
		subjectHash, reviewRoundID, gateResultID, decision, comment, reviewer, reviewedAt,
	)
}

func implementationRunRows(run delivery.ImplementationRun) *sqlmock.Rows {
	columns := []string{
		...
		"worktree_head", "patch_rel_path", "patch_sha256", "patch_bytes", "files_changed",
		"additions", "deletions", "files_json", "plan_deviations_json", "validation_results_json", "provider_summary", "change_set_created_at",
		"review_subject_hash", "review_round_id", "review_gate_result_id",
		"review_decision", "review_comment", "review_reviewer", "review_created_at",
	}
	var worktreeHead, patchRelPath, patchSHA256, patchBytes, filesChanged any
	var additions, deletions, filesJSON, deviationsJSON, validationsJSON, providerSummary, changeCreatedAt any
	var reviewSubjectHash, reviewRoundID, reviewGateResultID, reviewDecision, reviewComment any
	var reviewReviewer, reviewCreatedAt any
	if run.ChangeSet != nil {
		change := run.ChangeSet
		worktreeHead = change.WorktreeHead
		patchRelPath = change.PatchRelPath
		patchSHA256 = change.PatchSHA256
		patchBytes = change.PatchBytes
		...
		deviationsJSON, _ = json.Marshal(change.PlanDeviations)
		validationsJSON, _ = json.Marshal(change.ValidationResults)
		providerSummary = change.ProviderSummary
		changeCreatedAt = change.CreatedAt
	}
	if run.Review != nil {
		review := run.Review
		reviewSubjectHash = review.SubjectHash
		reviewRoundID = review.ReviewRoundID
		reviewGateResultID = review.GateResultID
	...
	return sqlmock.NewRows(columns).AddRow(
		...
		worktreeHead, patchRelPath, patchSHA256, patchBytes, filesChanged,
		additions, deletions, filesJSON, deviationsJSON, validationsJSON, providerSummary, changeCreatedAt,
		reviewSubjectHash, reviewRoundID, reviewGateResultID,
		reviewDecision, reviewComment, reviewReviewer, reviewCreatedAt,
	)
}

func implementationSummaryRows(run delivery.ImplementationRun) *sqlmock.Rows {
	columns := []string{
		"id", "request_id", "client_request_id", "request_hash", "design_artifact_id", "plan_artifact_id",
		"parent_run_id", "repo", "base_ref", "base_commit", "workspace_user_id", "workspace_username",
		"provider", "model", "provider_version", "network_enabled", "status", "worker_id", "lease_expires_at",
	...
	return sqlmock.NewRows(columns).AddRow(
		...
		nil, run.ProviderSessionID, nil, run.ErrorSummary, run.RequestedBy,
		nil, nil, nil, nil, run.CleanupError, run.CreatedAt,
		subjectHash, reviewRoundID, gateResultID, decision, comment, reviewer, reviewCreated,
	)
}
...
func TestSaveChangeSetAndFinishCommitsChangeAndTerminalStateTogether(t *testing.T) {
	...
	change := delivery.ChangeSet{
		...
		Files:             []delivery.ChangedFile{{Path: "main.go", Status: "M", Additions: 2, Deletions: 1}},
		PlanDeviations:    []delivery.PlanDeviation{},
		ValidationResults: []delivery.ValidationResult{{Sequence: 1, Status: "passed"}},
	...
	mock.ExpectQuery(`SELECT status FROM feature_implementation_runs WHERE id=\? LIMIT 1`).
		WithArgs("run-1").
		WillReturnRows(sqlmock.NewRows([]string{"status"}).AddRow(delivery.RunValidating))
	mock.ExpectExec(`(?s)UPDATE feature_implementation_runs.*worktree_head=\?.*status=\?.*WHERE id=\? AND status='validating'`).
		WithArgs(
			"abc", "run-1/changes.patch", "hash", int64(42), 1, 2, 1,
			...
			delivery.RunSucceeded, "", retainUntil, "run-1",
		).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()

	err = NewFeatureDeliveryStore(db).SaveChangeSetAndFinish(
		context.Background(), change, delivery.RunSucceeded, "", retainUntil,
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRequestCancelDistinguishesQueuedActiveAndTerminalRuns(t *testing.T) {
	tests := []struct {
		...
		wantStatus delivery.RunStatus
		wantErr    error
		update     string
	}{
		{name: "queued becomes terminal", status: delivery.RunQueued, wantStatus: delivery.RunCancelled, update: `status='cancelled'`},
		{name: "active records intent", status: delivery.RunRunning, wantStatus: delivery.RunRunning, update: `cancel_requested_at=COALESCE`},
		{name: "terminal conflicts", status: delivery.RunSucceeded, wantStatus: delivery.RunSucceeded, wantErr: delivery.ErrConflict},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			db, mock, err := sqlmock.New()
...
func TestListRunEventsUsesCursorAndBoundedLimit(t *testing.T) {
	...
	mock.ExpectQuery(query).
		...
		WillReturnRows(sqlmock.NewRows([]string{"run_id", "seq", "kind", "summary", "detail_json", "created_at"}).
			AddRow("run-1", int64(42), delivery.EventProviderMessage, "progress", []byte(`{"step":1}`), createdAt))
