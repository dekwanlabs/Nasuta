...
func NewExecutor(service *featuredelivery.Service) *Executor {
...
func (executor *Executor) generate(
	...
	if kind == featuredelivery.KindRequirementAnalysis {
		var pipelineRequest Request
		if err := json.Unmarshal(input.Payload, &pipelineRequest); err != nil {
			return agentworkflow.NodeResult{}, fmt.Errorf("decode pipeline request: %w", err)
		}
		...
		if _, ok := findArtifact(state.Artifacts, expectedParentKind(kind)); !ok ||
			...
			return agentworkflow.NodeResult{}, fmt.Errorf(
				...
				kind, featuredelivery.ErrConflict,
			)
		}
	...
	if err != nil {
		return agentworkflow.NodeResult{}, fmt.Errorf("generate %s: %w", kind, err)
	}
	if state.FeatureID == "" {
		state.FeatureID = featureID
	}
	if state.FeatureID != featureID {
		return agentworkflow.NodeResult{}, fmt.Errorf("pipeline feature id changed: %w", featuredelivery.ErrConflict)
	}
	if state.CurrentArtifact != nil && artifact.ParentArtifactID != state.CurrentArtifact.ID {
		return agentworkflow.NodeResult{}, fmt.Errorf(
			...
			artifact.ID, artifact.ParentArtifactID, state.CurrentArtifact.ID, featuredelivery.ErrConflict,
		)
	}
	...
	return agentworkflow.NodeResult{
		Handoff: agentworkflow.Handoff{Payload: payload, Completeness: agentworkflow.Complete},
		Usage: agentworkflow.WorkflowUsage{
...
func (executor *Executor) code(
	...
	if state.CurrentArtifact == nil || state.CurrentArtifact.Kind != featuredelivery.KindImplementationPlan {
		return agentworkflow.NodeResult{}, fmt.Errorf(
			"coding requires the implementation plan as current artifact: %w",
			featuredelivery.ErrConflict,
		)
	}
	design, ok := findArtifact(state.Artifacts, featuredelivery.KindSystemDesign)
	if !ok {
		return agentworkflow.NodeResult{}, fmt.Errorf(
			"coding requires a system design: %w", featuredelivery.ErrConflict,
		)
	}
	options := state.Options
	...
	if err != nil {
		return agentworkflow.NodeResult{}, fmt.Errorf("create implementation: %w", err)
	}
	state.Options = options
...
func (executor *Executor) validate(
	...
	if run.RequestID != state.FeatureID ||
		...
		return agentworkflow.NodeResult{}, fmt.Errorf(
			...
			run.ID, featuredelivery.ErrConflict,
		)
	}
	if run.Status != featuredelivery.RunSucceeded || run.ChangeSet == nil {
		return agentworkflow.NodeResult{}, fmt.Errorf(
			"implementation %q finished as %q without a change set: %w",
			run.ID, run.Status, featuredelivery.ErrConflict,
		)
	}
	...
	if err != nil {
		return agentworkflow.NodeResult{}, fmt.Errorf("marshal pipeline result: %w", err)
	}
	return agentworkflow.NodeResult{
		Handoff: agentworkflow.Handoff{Payload: payload, Completeness: agentworkflow.Complete},
	}, nil
...
func (executor *Executor) waitForImplementation(
	...
	for {
		...
		switch run.Status {
		case featuredelivery.RunSucceeded:
			return run, nil
		case featuredelivery.RunFailed, featuredelivery.RunCancelled, featuredelivery.RunInterrupted:
			return nil, fmt.Errorf(
				"implementation %q ended as %q: %w",
				run.ID, run.Status, featuredelivery.ErrConflict,
			)
		case featuredelivery.RunQueued, featuredelivery.RunPreparing,
			featuredelivery.RunRunning, featuredelivery.RunValidating:
...
func decodeState(payload json.RawMessage) (State, error) {
	...
	if state.FeatureID == "" || state.CurrentArtifact == nil {
		return State{}, fmt.Errorf("pipeline state is incomplete: %w", featuredelivery.ErrConflict)
	}
	return state, nil
}

func expectedParentKind(kind featuredelivery.ArtifactKind) featuredelivery.ArtifactKind {
	parent, _ := featuredelivery.ParentKind(kind)
	return parent
}

func findArtifact(artifacts []ArtifactSummary, kind featuredelivery.ArtifactKind) (ArtifactSummary, bool) {
...
func summarizeArtifact(artifact featuredelivery.Artifact) ArtifactSummary {
...
func summarizeChangeSet(changeSet featuredelivery.ChangeSet) ChangeSetSummary {
	...
	return ChangeSetSummary{
		...
		Files: files, PlanDeviations: append([]featuredelivery.PlanDeviation(nil), changeSet.PlanDeviations...),
...
func summarizeValidationResults(results []featuredelivery.ValidationResult) []ValidationSummary {
