...
func (workflowStore *Store) LoadDefinitionRollouts(
	...
	rows, err := workflowStore.db.QueryContext(ctx, `SELECT
		workflow_id,rule_version,candidate_version,percentage_bps,salt,rule_hash,
		active,created_by,created_at
		FROM workflow_definition_rollouts ORDER BY workflow_id`)
	if err != nil {
...
func (workflowStore *Store) SetDefinitionRollout(
	...
	err = tx.QueryRowContext(ctx, `SELECT rule_version
		FROM workflow_definition_rollouts
		WHERE workflow_id=? LIMIT 1 FOR UPDATE`,
		rule.WorkflowID,
	...
	if errors.Is(err, sql.ErrNoRows) {
		_, err = tx.ExecContext(ctx, `INSERT INTO workflow_definition_rollouts(
			workflow_id,rule_version,candidate_version,percentage_bps,salt,rule_hash,
			active,created_by,created_at)
			VALUES(?,?,?,?,?,?,?,?,?)`,
			rule.WorkflowID, rule.RuleVersion, rule.CandidateVersion,
		...
		_, err = tx.ExecContext(ctx, `UPDATE workflow_definition_rollouts SET
			rule_version=?,candidate_version=?,percentage_bps=?,salt=?,rule_hash=?,
			active=?,created_by=?,created_at=? WHERE workflow_id=?`,
...
func (workflowStore *Store) ListDefinitionRolloutAudit(
	...
	rows, err := workflowStore.db.QueryContext(ctx, `SELECT
		seq,workflow_id,rule_version,candidate_version,percentage_bps,rule_hash,
		action,actor_user_id,created_at
		FROM workflow_definition_rollout_audit
...
func appendWorkflowDefinitionAuditTx(
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO workflow_definition_audit(
...
func appendWorkflowDefinitionRolloutAuditTx(
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO workflow_definition_rollout_audit(
