...
func TestStoreSetRolloutInsertsInitialRuleAndAudit(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)SELECT rule_version\s+FROM catalog_rollouts\s+WHERE catalog_kind='agent' AND subject_id=\? LIMIT 1 FOR UPDATE`).
		WithArgs(rule.AgentID).
		WillReturnRows(sqlmock.NewRows([]string{"rule_version"}))
	mock.ExpectExec(`(?s)INSERT INTO catalog_rollouts\(`).
		WithArgs(
			rule.AgentID, rule.RuleVersion, rule.AgentID, rule.CandidateVersion,
...
func TestStoreSetRolloutUpdatesNextRuleAndAudit(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE catalog_rollouts SET\s+rule_version=\?,candidate_id=\?,candidate_version=\?,percentage_bps=\?,salt=\?,rule_hash=\?,\s+active=\?,created_by=\?,created_at=\?\s+WHERE catalog_kind='agent' AND subject_id=\?`).
		WithArgs(
			rule.RuleVersion, rule.AgentID, rule.CandidateVersion, rule.PercentageBPS,
...
func TestStoreListRolloutAuditUsesBoundedCursor(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT\s+seq,subject_id,version,candidate_version,percentage_bps,rule_hash,\s+action,actor_user_id,created_at\s+FROM catalog_audit\s+WHERE catalog_kind='agent' AND event_kind='rollout'\s+AND subject_id=\? AND seq>\?\s+ORDER BY seq LIMIT \?`).
