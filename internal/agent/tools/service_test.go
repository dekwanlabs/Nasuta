...
func TestSemanticServiceNamesReportsProviderFailures(t *testing.T) {
	t.Run("embed error", func(t *testing.T) {
		svc := New(Deps{Embedder: failingEmbedder{err: errors.New("provider unavailable")}})
	...
	t.Run("empty embedding", func(t *testing.T) {
		svc := New(Deps{Embedder: emptyEmbedder{}})
	...
	t.Run("search error", func(t *testing.T) {
		...
		svc := New(Deps{Semantic: semanticStore, Embedder: testEmbedder{}})
...
func TestFindCodeReportsEmptyQueryEmbedding(t *testing.T) {
	svc := New(Deps{
		Semantic: &searchFallbackSemantic{},
		Embedder: emptyEmbedder{},
	})

	_, err := svc.FindCode(context.Background(), "orders", "", 5)
	if err == nil || !strings.Contains(err.Error(), errEmptyQueryEmbedding.Error()) {
		t.Fatalf("FindCode error = %v", err)
	}
}
...
func TestCodeSearchUsesDenseFallbackWhenBM25HasNoKnownTerms(t *testing.T) {
	...
	svc := New(Deps{Semantic: semantic, Embedder: testEmbedder{}})
...
func TestCodeSearchKeepsRRFSeparateFromCosine(t *testing.T) {
	...
	svc := New(Deps{Semantic: &fusionSemantic{score: semanticHybrid}, Embedder: testEmbedder{}})
