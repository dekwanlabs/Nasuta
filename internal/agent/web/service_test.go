...
func TestSetWebSearchEngineCanonicalizesValue(t *testing.T) {
	...
	if srv.searchEngine != "bing" {
		t.Fatalf("searchEngine = %q, want %q", srv.searchEngine, "bing")
	}
}
...
func TestBlockedFetchIP(t *testing.T) {
	tests := []struct {
		...
		{address: "169.254.1.1", blocked: true},
	...
	for _, test := range tests {
		t.Run(test.address, func(t *testing.T) {
			if got := blockedFetchIP(net.ParseIP(test.address)); got != test.blocked {
				t.Fatalf("blockedFetchIP(%q) = %v, want %v", test.address, got, test.blocked)
			}
		})
	}
}
