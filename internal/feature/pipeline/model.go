// Package pipeline adapts Feature Delivery to the generic Workflow runtime.
package pipeline
...
type ImplementationSummary struct {
	ID              string             `json:"id"`
	ClientRequestID string             `json:"client_request_id"`
	Status          delivery.RunStatus `json:"status"`
}
...
type ChangeSetSummary struct {
	...
	PlanDeviations    []delivery.PlanDeviation `json:"plan_deviations"`
...
func normalizeRequest(request Request) (Request, error) {
	...
	repository, err := delivery.NormalizeRepository(request.Repository)
	if err != nil {
		return Request{}, err
	}
	request.Repository = repository
	if request.FeatureID == "" {
		return Request{}, fmt.Errorf("feature_id is required: %w", delivery.ErrInvalid)
	}
	if request.ClientRequestID == "" {
		return Request{}, fmt.Errorf("client_request_id is required: %w", delivery.ErrInvalid)
	}
	if len(request.ClientRequestID) > 128 {
		return Request{}, fmt.Errorf("client_request_id exceeds 128 bytes: %w", delivery.ErrInvalid)
	}
	if request.BaseRef == "" {
		request.BaseRef = "HEAD"
	}
	if request.Provider == "" {
		return Request{}, fmt.Errorf("provider is required: %w", delivery.ErrInvalid)
	}
	return request, nil
}
...
func (options RequestOptions) implementationOptions() delivery.ImplementationOptions {
	return delivery.ImplementationOptions{
