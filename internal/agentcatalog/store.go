...
func (catalogStore *Store) LoadRollouts(
	...
	rows, err := catalogStore.db.QueryContext(ctx, `SELECT
		agent_id,rule_version,candidate_version,percentage_bps,salt,rule_hash,
		active,created_by,created_at
		FROM agent_definition_rollouts ORDER BY agent_id`)
	if err != nil {
...
func (catalogStore *Store) SetRollout(
	...
	err = tx.QueryRowContext(ctx, `SELECT rule_version
		FROM agent_definition_rollouts WHERE agent_id=? LIMIT 1 FOR UPDATE`,
		rule.AgentID,
	...
	if errors.Is(err, sql.ErrNoRows) {
		_, err = tx.ExecContext(ctx, `INSERT INTO agent_definition_rollouts(
			agent_id,rule_version,candidate_version,percentage_bps,salt,rule_hash,
			active,created_by,created_at)
			VALUES(?,?,?,?,?,?,?,?,?)`,
			rule.AgentID, rule.RuleVersion, rule.CandidateVersion, rule.PercentageBPS,
			rule.Salt, rule.RuleHash, rule.Active, actorUserID,
			platformstore.DatabaseTime(createdAt.Format(time.RFC3339Nano)),
		)
	} else {
		_, err = tx.ExecContext(ctx, `UPDATE agent_definition_rollouts SET
			rule_version=?,candidate_version=?,percentage_bps=?,salt=?,rule_hash=?,
			active=?,created_by=?,created_at=? WHERE agent_id=?`,
			rule.RuleVersion, rule.CandidateVersion, rule.PercentageBPS, rule.Salt,
...
func (catalogStore *Store) ListAudit(
	...
	rows, err := catalogStore.db.QueryContext(ctx, `SELECT
		seq,definition_id,version,action,actor_user_id,created_at
		FROM agent_definition_audit
		WHERE definition_id=? AND seq>?
...
func (catalogStore *Store) ListRolloutAudit(
	...
	rows, err := catalogStore.db.QueryContext(ctx, `SELECT
		seq,agent_id,rule_version,candidate_version,percentage_bps,rule_hash,
		action,actor_user_id,created_at
		FROM agent_definition_rollout_audit
...
func appendAuditTx(
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO agent_definition_audit(
		definition_id,version,action,actor_user_id,created_at)
		VALUES(?,?,?,?,?)`,
...
func appendRolloutAuditTx(
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO agent_definition_rollout_audit(
